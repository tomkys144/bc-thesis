...
Přístroje pro měření \glref{EMG} měří změny potenciálu v~jednotlivých svalech. Mohou být buď hodně obecné\rfc{Přeformulovat}, či měřit konkrétní svalová vlákna. To záleží primárně na použité elektrodě a na filtrovaných frekvencích. Samotný přístroj je v~podstatě voltmetr, který měří napětí velká řádově milivolty a následně je zesiluje na vhodné velikosti a potlačuje šum. Cesta signálu je poté z~elektrod přes diferenciální zesilovač a filtry do AD převodníku.
...
\rfc{Upravit tabulku/dovysvětlit proč my 256 Hz}
...
Toto umístění slouží k~sledování nespecifického\rfc{Přeformulovat} pnutí v~předloktí. Využívá se toho k~analýze zranění horní končetiny, např. z~repetitivních činností.
